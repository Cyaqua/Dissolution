...
public abstract class BlockGenericContainer extends Block {
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if (heldItem.isEmpty() && playerIn.isSneaking()) {
            if (!worldIn.isRemote) {
                playerIn.inventory.addItemStackToInventory(getDroppedItem(worldIn, pos));
            ...
            if (tileItemInventory != null) {
                if (heldItem.isEmpty()) {
                    playerIn.inventory.addItemStackToInventory(tileItemInventory.extractItem(0, TileEntityCrucible.MAX_VOLUME, false));
                    return true;
