...
*/
    ...
    public void onPlayerRespawn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerRespawnEvent event) {
        ...
        if (playerCorp.getDeathStats().wasDead() && !event.player.world.isRemote) {
            event.player.world.theProfiler.startSection("placing_respawned_player");
            ...
            if (!DissolutionConfig.respawn.wowLikeRespawn) {
                ...
                if (deathPos.getY() > 0 && event.player.world.isAirBlock(deathPos))
            ...
            event.player.world.theProfiler.endSection();
    ...
    public void onLivingAttack(LivingAttackEvent event) {
        if (event.getEntity() instanceof EntityPlayer && !event.getSource().canHarmInCreative()) {
            ...
            if (status.isIncorporeal()) {
                if (event.getSource().getSourceOfDamage() == null || !DissolutionConfigManager.canEctoplasmBeAttackedBy(event.getSource().getSourceOfDamage()))
                    event.setCanceled(!event.getSource().canHarmInCreative());
            }
        } else {
            IIncorporealHandler handler = CapabilityIncorporealHandler.getHandler(event.getSource().getSourceOfDamage());
