...
public abstract class AbstractMinion extends EntityMob implements IRangedAttackMob, IEntityOwnable, IPossessable {
    ...
    public boolean isEntityInvulnerable(@Nonnull DamageSource source) {
        return (source.getEntity() == null && this.isInert() && !source.canHarmInCreative()) || super.isEntityInvulnerable(source);
    }

    protected void attractAttention() {
        List<EntityCreature> nearby = this.world.getEntitiesWithinAABB(EntityCreature.class,
                new AxisAlignedBB(new BlockPos(this)).expandXyz(30), this::isMobEligibleForAttention);
    ...
    public boolean onEntityPossessed(EntityPlayer player) {
        ...
        for (EntityEquipmentSlot slot : EntityEquipmentSlot.values())
            ...
            else player.inventory.addItemStackToInventory(this.getItemStackFromSlot(slot));
    ...
    public boolean proxyAttack(EntityLivingBase entity, DamageSource source, float amount) {
        ...
        if (source instanceof EntityDamageSourceIndirect)
            //noinspection ConstantConditions
            newSource = new EntityDamageSourceIndirect(source.getDamageType(), source.getSourceOfDamage(), this);
...
//        Entity entity = source.getTrueSource();
    ...
    public void updatePassenger(@Nonnull Entity passenger) {
        ...
        if (passenger.getUniqueID().equals(this.getPossessingEntity())) {
            ...
            if (passenger instanceof EntityPlayer) {
                for (PotionEffect potionEffect : ((EntityPlayer) passenger).getActivePotionEffects())
    ...
    protected EntityEquipmentSlot getClickedSlot(Vec3d raytrace) {
        ...
        double d0 = (this.isInert() ? raytrace.zCoord + 1.2 : raytrace.yCoord) * (flag ? 2.0D : 1.0D);
