...
public class EntityFleetingSoul extends AbstractSoul {
    ...
    public void onUpdate() {
        ...
        if (!this.world.isRemote && !this.isDead) {
            ...
            if (this.soulAge % 100 == 0)
                if (!(this.targetEntity instanceof EntityPlayer) || this.getDistanceSq(new BlockPos(targetEntity)) > 1024.0
                    ...
                    if (targetEntity == null) {
                        List<EntityItem> items = this.world.getEntitiesWithinAABB(EntityItem.class, new AxisAlignedBB(getPosition()).expandXyz(16),
                                item -> item != null && item.getEntityItem().getItem() == ModItems.HALITE);
                        if (items.size() > 0)
            ...
            if (length > innerRange && length < maxRange)
                ...
                weight = 1;
            motionX = (1 - weight) * ((0.9) * motionX + (0.1) * targetVector.xCoord) + weight * targetVector.zCoord;
            motionY = (1 - weight) * ((0.9) * motionY + (0.1) * targetVector.yCoord) + weight * targetVector.yCoord;
            motionZ = (1 - weight) * ((0.9) * motionZ + (0.1) * targetVector.zCoord) - weight * targetVector.xCoord;
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn) {
        ...
        if (!world.isRemote && !bottle.isEmpty() && this.delayBeforeCanPickup <= 0) {
            bottle.shrink(1);
            entityIn.inventory.addItemStackToInventory(ItemSoulInAJar.newTypedSoulBottle(this.soul.getType()));
            this.setDead();
        }
    }
