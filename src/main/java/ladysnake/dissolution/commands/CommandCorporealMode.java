package ladysnake.dissolution.commands;

import ladysnake.dissolution.Dissolution;
import ladysnake.dissolution.soulstates.CapabilitySoulState;
import ladysnake.dissolution.soulstates.ISoulState;
import ladysnake.dissolution.soulstates.ISoulStateHandler;
import ladysnake.dissolution.soulstates.ModSoulStates;
...
public class CommandCorporealMode extends CommandBase {
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        ...
        if (args.length == 1) {
            ...
            possibilities = ModSoulStates.REGISTRY.getValues().stream().map(status -> Objects.requireNonNull(status.getRegistryName()).getResourcePath()).collect(Collectors.toList());
    ...
    public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender sender, @Nonnull String[] args) throws CommandException {
        ...
        if ("query".equals(args[0]) || "get".equals(args[0])) {                 // dissolution cmode query  -> returns the player's current status
            ...
            CapabilitySoulState.getHandler(player).ifPresent(handler -> notifyCommandListener(sender, this,
                    "commands.dissolution.corporeality_mode.query." + (player == sender ? "self" : "other"),     // different feedback
                    player.getName(), new TextComponentTranslation(handler.getCurrentState().toString())));
            ...
            try {
                ResourceLocation regName = args[1].contains(":")
                        ...
                        : new ResourceLocation(Dissolution.MODID, args[1]);
                ISoulState newStatus = Objects.requireNonNull(ModSoulStates.REGISTRY.getValue(regName));
...
//                    throw new CommandException("commands.dissolution.corporeality_mode.set.weak_soul", player.getName());
                ...
                if (args.length >= 4) {
                    ...
                    try {
                        NBTTagCompound nbt = JsonToNBT.getTagFromJson(s1);
                        newStatus.readData(player, nbt);
                    } catch (NBTException nbtexception) {
                        throw new CommandException("commands.summon.tagError", nbtexception.getMessage());
                    }
                }
                ITextComponent iTextComponent = new TextComponentTranslation(newStatus.toString());
